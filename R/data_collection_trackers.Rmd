...
df_data_support_cl_log <- df_tool_data |> 
    select(uuid, zone1, woreda1, latitude, longitude)

df_cl_log <- read_csv(file = "../inputs/combined_checks_eth_jrma_somali.csv") |> 
    left_join(df_data_support_cl_log |> select(-woreda1), by = "uuid")
...
# sample data
df_samples_required <- readxl::read_excel("../support_files/jrma_samples_size.xlsx") |> 
    select(woreda1, required_samples = woreda_sample_size)
...
df_updated_tool_data |> 
    ...
    arrange(woreda1) |> 
    full_join(df_samp_per_location, by = c("woreda1" = "woreda1")) |>
    mutate(required_samples = ifelse(is.na(required_samples), 0, required_samples)) |> 
    left_join(df_cl_surveys_for_deletion, by = "woreda1") |> 
...
df_enum_performance |> 
    group_by(zone1, woreda1, start_date, enumerator_id) |> 
...
df_cl_log |> 
    ...
    group_by(uuid, woreda1, latitude, longitude) |> 
